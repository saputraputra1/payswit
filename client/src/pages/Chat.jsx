          ...
          <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-3 sm:space-y-4">
            ...
            {messages.length === 0 && (
              <div className="text-center py-8 sm:py-12">
                ...
                <p className="text-xs text-gray-600 mb-4">Ketik pesan atau pilih pertanyaan di bawah</p>
                <div className="flex flex-wrap justify-center gap-2">
                  {quickMessages.map((msg, i) => (
                    <button
                      key={i}
                      onClick={() => setNewMessage(msg)}
                      className="px-3 py-1.5 bg-white/[0.03] border border-white/10 rounded-lg text-xs text-gray-400 hover:text-white hover:bg-white/[0.06] transition-all"
                    >
                      {msg}
                    </button>
                  ))}
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] sm:max-w-[70%] ${msg.sender === 'user' ? 'order-1' : 'order-1'}`}>
                  {msg.sender !== 'user' && (
            ...
            <div className="px-3 sm:px-4 pb-2 flex flex-wrap gap-1.5">
              {quickMessages.map((msg, i) => (
                <button
                  key={i}
                  onClick={() => setNewMessage(msg)}
                  className="px-2.5 py-1 bg-white/[0.03] border border-white/10 rounded-lg text-[10px] sm:text-xs text-gray-500 hover:text-white hover:bg-white/[0.06] transition-all"
                >
                  {msg}
                </button>
              ))}
