...
import {
  ...
  FiShield, FiLock, FiGlobe, FiRefreshCw,
} from 'react-icons/fi'
import { BankIcon } from '../components/Icons'

const nominalList = [100000, 250000, 500000, 1000000, 2500000, 5000000]

// Spread untuk kurs jual (user beli USD, admin jual)
const SELL_SPREAD = 100

export default function Topup() {
  const { profile } = useAuth()
  const [nominal, setNominal] = useState('')
  const [baseRate, setBaseRate] = useState(null) // usdToIdr realtime
  ...
  // Bank accounts admin (untuk instruksi transfer)
  useEffect(() => {
    api
      .get('/bank-accounts')
      .then((res) => {
  ...
  const sellRate = baseRate ? baseRate + SELL_SPREAD : 15000
  const nominalNum = nominal ? parseFloat(nominal) : 0
  const usd = nominalNum ? nominalNum / sellRate : 0
  ...
  const handleTopup = async (e) => {
    ...
    setLoading(true)
    try {
      const usdNum = nominalNum / sellRate
      ...
      setLastTx({ id: data?.id, nominal: nominalNum, usd: usdNum })
              ...
              {selectedBank ? (
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => copyText(selectedBank.bankName, 'bank')}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => copyText(selectedBank.accountNumber, 'norek')}
                      className="w-9 h-9 rounded-lg bg-blue-500/10 flex items-center justify-center text-blue-400 hover:bg-blue-500/20 transition-colors"
                    >
          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-4 sm:p-6">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between"><span className="text-gray-500">Kurs jual</span><span className="text-white">Rp {sellRate.toLocaleString('id-ID')}</span></div>
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-4 sm:p-6">
              <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-xs text-green-400/70 font-medium">Kurs jual realtime</p>
                    <p className="text-sm font-bold text-green-400">
                      1 USD = Rp {sellRate.toLocaleString('id-ID')}
                    </p>
                  </div>
                </div>
            ...
            <div className="bg-white/[0.02] border border-white/[0.06] rounded-xl p-4 sm:p-6">
              ...
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-white">Rp {nominalNum ? nominalNum.toLocaleString('id-ID') : '0'}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-500">Kurs jual</span>
                  <span className="text-white">Rp {sellRate.toLocaleString('id-ID')}</span>
                </div>
                <div className="flex justify-between pt-3 border-t border-white/[0.06]">
                  <span className="text-gray-400 font-medium">PayPal masuk</span>
                  <span className="text-blue-400 font-bold">${usd.toFixed(2)}</span>
                </div>
              </div>
              {nominalNum > 0 && (
